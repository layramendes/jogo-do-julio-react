...
const NavIcon = ({ type }) => {
    ...
    return (<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">{icons?.[type]}</svg>);
};
...
const JulioCharacter = ({ isSleeping, status }) => {
    ...
    if (isSleeping) { animationState = 'sleeping'; } 
    else if (status?.hunger < 30 || status?.fun < 30 || status?.hygiene < 30) { animationState = 'sad'; }
    return (<motion.img src={julioImage} alt="Julio" className="w-48 h-48 object-contain" variants={julioVariants} animate={animationState} />);
};
        ...
        const unsubscribe = onSnapshot(playerDocRef, (doc) => {
            if (!doc.exists()) {
                const initialState = {
                    ...
                    coins: 100, isSleeping: false, customization: { background: 'bg-green-200' },
...
const Overlay = ({ title, onClose, children }) => (
    <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "tween", ease: "easeInOut", duration: 0.4 }} className="absolute bottom-0 left-0 right-0 h-[75%] bg-white/80 backdrop-blur-md rounded-t-3xl p-4 flex flex-col shadow-2xl z-20">
        <div className="flex justify-between items-center mb-4 flex-shrink-0"><h2 className="text-2xl font-bold text-gray-700">{title}</h2><button onClick={onClose} className="bg-gray-200 p-2 rounded-full hover:bg-gray-300 transition-colors"><svg className="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button></div>
...
const Shop = ({ playerState, updatePlayerState, onBuyCoins, onShowFeedback }) => {
    const handleBuyItem = (itemId) => {
        ...
        if (!item || playerState?.coins < item.price) { onShowFeedback("Moedas insuficientes!"); return; }
        const newCoins = playerState.coins - item.price;
        if (item.type === 'background') { updatePlayerState({ coins: newCoins, backgroundsOwned: arrayUnion(item.value) }); } 
        else { const currentAmount = playerState?.inventory?.[itemId] || 0; updatePlayerState({ coins: newCoins, [`inventory.${itemId}`]: currentAmount + 1 }); }
        onShowFeedback(`${item.name} comprado!`);
    };
    return (
        <div>
            <h3 className="text-lg font-semibold text-gray-600 mb-3">Comprar Itens</h3><div className="grid grid-cols-2 md:grid-cols-3 gap-3">{Object.entries(gameItems).map(([id, item]) => (<ItemCard key={id} onClick={() => handleBuyItem(id)} disabled={playerState?.coins < item.price}><div className="text-4xl mb-2">{item.icon}</div><p className="font-semibold text-gray-700">{item.name}</p><div className="flex items-center justify-center mt-2 text-amber-600 font-bold">💰<span className="ml-1">{item.price}</span></div></ItemCard>))}</div>
            <h3 className="text-lg font-semibold text-gray-600 mt-6 mb-3">Comprar Moedas</h3><div className="grid grid-cols-1 gap-3">{coinPackages.map((pkg) => (<div key={pkg.id} onClick={() => onBuyCoins(pkg.id)} className="bg-gradient-to-br from-amber-300 to-yellow-400 p-4 rounded-xl shadow-lg flex items-center justify-between text-white cursor-pointer hover:from-amber-400 hover:to-yellow-500 transition-all"><div><div className="text-2xl">{pkg.icon}</div><p className="font-bold text-lg">{pkg.name}</p></div><div className="bg-white/30 text-white font-bold py-2 px-4 rounded-full">{pkg.price}</div></div>))}</div>
        </div>
    );
};

const DraggableItem = ({ item, onDragEnd, constraintsRef }) => (<motion.div className="absolute text-6xl z-50 cursor-grab" style={{ top: '50%', left: '50%', x: '-50%', y: '-50%' }} drag dragConstraints={constraintsRef} dragElastic={0.5} onDragEnd={(event, info) => onDragEnd(info, item)} whileDrag={{ scale: 1.2, cursor: 'grabbing' }}>{item.icon}</motion.div>);
const InteractionFeedback = ({ text }) => (<motion.div key={Date.now()} className="absolute z-50 text-2xl font-bold text-yellow-500 pointer-events-none" style={{ textShadow: '1px 1px 2px black' }} initial={{ y: 0, opacity: 1, scale: 0.8 }} animate={{ y: -50, opacity: 0, scale: 1.2 }} transition={{ duration: 1.5, ease: "easeOut" }}>{text}</motion.div>);
const DirtPatch = ({ patchRef }) => (<motion.div ref={patchRef} className="absolute w-8 h-8 bg-yellow-900/60 rounded-full z-40" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} transition={{ duration: 0.3 }} />);
const CleaningMinigame = ({ onFinish }) => {
    const [patches, setPatches] = useState([]);
    const soapRef = useRef(null);
    const patchRefs = useRef([]);
    useEffect(() => { const newPatches = Array.from({ length: 5 }).map((_, i) => ({ id: i, style: { top: `${Math.random() * 60 + 10}%`, left: `${Math.random() * 70 + 10}%` } })); setPatches(newPatches); patchRefs.current = newPatches.map(() => React.createRef()); }, []);
    const handleScrub = () => { if (!soapRef.current) return; const soapRect = soapRef.current.getBoundingClientRect(); let patchesWereCleaned = false; const remainingPatches = patches.filter((patch, index) => { const patchEl = patchRefs.current[index]?.current; if (!patchEl) return true; const patchRect = patchEl.getBoundingClientRect(); const isColliding = !(soapRect.right < patchRect.left || soapRect.left > patchRect.right || soapRect.bottom < patchRect.top || soapRect.top > patchRect.bottom); if(isColliding) { patchesWereCleaned = true; return false; } return true; }); if (patchesWereCleaned) { setPatches(remainingPatches); if (remainingPatches.length === 0) { onFinish(); } } };
    return (<div className="absolute inset-0 z-30"><motion.div ref={soapRef} className="absolute text-6xl z-50 cursor-grab" style={{ top: '70%', left: '40%' }} drag dragConstraints={{ top: 0, left: 0, right: 0, bottom: 0 }} dragElastic={1} onDrag={handleScrub} whileDrag={{ scale: 1.2, cursor: 'grabbing' }}>🧼</motion.div><AnimatePresence>{patches.map((patch, index) => (<div key={patch.id} style={patch.style}><DirtPatch patchRef={patchRefs.current[index]} /></div>))}</AnimatePresence></div>);
};

const RoomComponent = ({ playerState, itemType, emptyMessage, onUseItem }) => {
    const items = Object.keys(playerState?.inventory || {}).filter(key => gameItems?.[key]?.type === itemType && playerState.inventory?.[key] > 0);
    ...
    return (<div className="grid grid-cols-2 md:grid-cols-3 gap-3">{items.map(itemId => (<ItemCard key={itemId} onClick={() => onUseItem(itemId)}><div className="text-4xl mb-2">{gameItems?.[itemId]?.icon}</div><p className="font-semibold text-gray-700">{gameItems?.[itemId]?.name}</p><p className="text-sm text-gray-500">Quantidade: {playerState.inventory?.[itemId]}</p></ItemCard>))}</div>);
};

const Bedroom = ({ playerState, updatePlayerState, onSleep }) => {
    const { backgroundsOwned = [], customization } = playerState || {};
    const handleSetBackground = (bgClass) => { updatePlayerState({ 'customization.background': bgClass }); };
    return (<div><button onClick={onSleep} className="w-full bg-indigo-500 text-white font-bold py-3 px-4 rounded-xl mb-6 hover:bg-indigo-600 transition-colors shadow-lg">{playerState?.isSleeping ? 'Acordar Julio 🌙' : 'Pôr Julio a Dormir ☀️'}</button><h3 className="text-lg font-semibold text-gray-600 mb-3">Mudar Fundo</h3><div className="grid grid-cols-2 gap-3">{backgroundsOwned?.map(bg => (<div key={bg} onClick={() => handleSetBackground(bg)} className={`h-24 rounded-lg cursor-pointer border-4 ${customization?.background === bg ? 'border-amber-400' : 'border-transparent'} ${bg}`}></div>))}</div></div>);
};

// NOVO COMPONENTE: MINI-JOGO "APANHA A BOLA"
const CatchTheBallGame = ({ onFinish }) => {
    ...
    if (gameState === 'ready') {
        return (
            <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center z-50 text-white p-4 text-center">
    ...
    return (
        <div className="absolute inset-0 bg-sky-300/80 z-50">
            <div className="p-4 text-white font-bold text-xl flex justify-between">
...
export default function App() {
    ...
    const [isGameActive, setIsGameActive] = useState(false); // NOVO ESTADO DO MINI-JOGO
    ...
    const gameLoop = useCallback(() => {
        if (!playerState || isCleaning || isGameActive) return; // Pausa o decaimento durante atividades
        let newStats = { ...playerState.stats };
        if (playerState.isSleeping) { newStats.energy = Math.min(100, newStats.energy + 1.0); } 
        else {
            newStats.hunger = Math.max(0, newStats.hunger - 0.5); newStats.hygiene = Math.max(0, newStats.hygiene - 0.3);
            newStats.fun = Math.max(0, newStats.fun - 0.4); newStats.energy = Math.max(0, newStats.energy - 0.2);
            if (newStats.hunger < 20 || newStats.hygiene < 20) { newStats.health = Math.max(0, newStats.health - 0.5); }
        }
        updatePlayerState({ stats: newStats });
    }, [playerState, updatePlayerState, isCleaning, isGameActive]);
    ...
    const handleBuyCoins = useCallback(async (priceId) => { if (!priceId || priceId.includes('SEU_PRICE_ID')) { alert("Erro: O ID de preço do Stripe não foi configurado."); return; } const functions = getFunctions(getApp()); const createStripeCheckout = httpsCallable(functions, 'createStripeCheckout'); try { const { data } = await createStripeCheckout({ priceId, success_url: window.location.href, cancel_url: window.location.href }); const stripe = await window.Stripe(stripePublicKey); await stripe.redirectToCheckout({ sessionId: data?.id }); } catch (error) { console.error("Erro no Stripe Checkout:", error); alert("Ocorreu um erro ao iniciar a compra."); } }, []);
    ...
    const handleUseItem = (itemId) => {
        ...
        if (item.type === 'hygiene') { updatePlayerState({ [`inventory.${itemId}`]: currentAmount - 1 }); setIsCleaning(true); setActiveOverlay(null); } 
    ...
    const handleItemDrop = (info, item) => {
        ...
        if (draggedPoint.x > julioRect.left && draggedPoint.x < julioRect.right && draggedPoint.y > julioRect.top && draggedPoint.y < julioRect.bottom) {
    ...
    const handleGameFinish = (score) => {
        ...
        updatePlayerState({
            coins: playerState.coins + coinsEarned,
        });
    ...
    const Playroom = () => (
        <div>
            ...
            <ItemCard onClick={() => { setIsGameActive(true); setActiveOverlay(null); }}>
    ...
    const renderOverlayContent = () => {
        ...
        switch (activeOverlay) {
            ...
            case 'Sala de Jogos': return <Playroom />;
    ...
    return (
        <div className={`w-full max-w-sm mx-auto h-[95vh] max-h-[800px] flex flex-col font-sans overflow-hidden ${backgroundClass} rounded-3xl shadow-2xl p-4 border-4 border-amber-300 transition-colors duration-500`}>
            <header className="flex justify-between items-center mb-2 flex-shrink-0"><h1 className="text-xl font-bold text-gray-700">A Cuidar do Julio</h1><div className="text-lg bg-yellow-400 text-white px-4 py-1 rounded-full shadow flex items-center">💰<span className="ml-2 font-semibold">{playerState?.coins}</span></div></header>
            <div className="grid grid-cols-[20px,1fr] gap-x-2 gap-y-1 items-center mb-4 flex-shrink-0 text-pink-600">
                <span>❤️</span><StatusBar value={playerState?.stats?.health} colorClass="bg-pink-500" /><span>🍖</span><StatusBar value={playerState?.stats?.hunger} colorClass="bg-orange-500" /><span>🧼</span><StatusBar value={playerState?.stats?.hygiene} colorClass="bg-cyan-500" /><span>🎾</span><StatusBar value={playerState?.stats?.fun} colorClass="bg-yellow-500" /><span>⚡</span><StatusBar value={playerState?.stats?.energy} colorClass="bg-lime-500" />
            </div>
            
            <main ref={mainRef} className="flex-grow relative flex flex-col items-center justify-center my-2 overflow-hidden">
                <AnimatePresence>
                    {isGameActive && <CatchTheBallGame onFinish={handleGameFinish} />}
                </AnimatePresence>
                
                <div className={`relative flex items-center justify-center transition-opacity duration-300 ${isGameActive ? 'opacity-0' : 'opacity-100'}`}>
                    <div ref={julioRef} className={`transition-all duration-300 ${isCleaning ? 'saturate-50' : ''}`}><JulioCharacter isSleeping={playerState?.isSleeping} status={playerState?.stats} /></div>
                </div>
                
                {isCleaning && <CleaningMinigame onFinish={handleEndCleaning} />}
                ...
                <div className={`absolute bottom-0 text-center mb-4 transition-opacity duration-300 ${isGameActive ? 'opacity-0' : 'opacity-100'}`}><p className="text-gray-600 font-semibold bg-white/50 px-3 py-1 rounded-full">{playerState?.isSleeping ? "Julio está a dormir... Zzz" : "Relaxa com o Julio!"}</p></div>
            ...
            <nav className="grid grid-cols-6 gap-2 mt-4 flex-shrink-0">
                <NavButton type="home" onClick={() => setActiveOverlay(null)} /><NavButton type="kitchen" onClick={() => setActiveOverlay('Cozinha')} /><NavButton type="bathroom" onClick={() => setActiveOverlay('Casa de Banho')} /><NavButton type="playroom" onClick={() => setActiveOverlay('Sala de Jogos')} /><NavButton type="bedroom" onClick={() => setActiveOverlay('Quarto')} /><NavButton type="shop" onClick={() => setActiveOverlay('Loja')} />
            </nav>
        </div>
    );
}